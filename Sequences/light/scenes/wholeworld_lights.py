...
def wholeworld_intro(seq, timer):
    seq.send(qlcport, '/TuttiProche/White/Segment/{1,2,8}', 176)#200)
    seq.send(qlcport, '/TuttiLointain/Red/Segment/{4,5}', 255)
    seq.send(qlcport, '/TuttiLointain/{Blue,Green}/Segment/{4,5}', 176)#200)
...
def wholeworld_faded_gbflash(seq,timer):
    eased_fade(seq, timer, '/TuttiProche/{Blue,Green}/Segment/{3,4,5,6,7}', 109, 0, 0.12*60/seq.bpm, 0.01)
...
def wholeworld_couplet(seq,timer):
    crepitement(seq, timer, [['TuttiProche', 'White', '1', 176, 180], ['TuttiProche', 'White', '{2,8}', 176, 180], ['TuttiLointain', '{Green,Blue}', '{4,5}', 141, 148], ['TuttiLointain', 'Red', '{4,5}', 176, 180]])

def wholeworld_synthflash(seq,timer):
    eased_fade(seq, timer, '/TuttiLointain/Red/Segment/{1,2,3,6,7,8}', 161, 0, 0.25*60/seq.bpm, 0.01)
    eased_fade(seq, timer, '/TuttiLointain/{Green,Blue}/Segment/{1,2,3,6,7,8}', 109, 0, 0.25*60/seq.bpm, 0.01)

def wholeworld_refrain_fixe(seq,timer):
    crepitement(seq, timer, [['TuttiLointain', '{Red,Green}', '{4,5,8}', 70, 141]])
...
def wholeworld_boomboclaat(seq,timer):
    strobelights(seq, timer, ['ProcheJardin','ProcheCour'], ['Red'], [1, 2, 3, 6, 7, 8], 'aleatoire', ramped = [46, 161, 9], step = 0.25*60/seq.bpm/4, alea_type = ['tutti', 1, 1, 3])
